...
2. You can modify your scenario request JSON data at runtime by using `request_json_modifiers` from `TestCaseRunner.run`.
    ...
    - Note that this is not the same as JsonPath expressions (e.g. https://github.com/json-path/JsonPath)
...
`rest-api-tester` can also auto-update your scenario files for you when test fails.
...
- Setting `rest_api_tester.config.Config.UPDATE_EXPECTATIONS_ON_FAIL = true`.
- Setting `self.update_expectations_on_fail = True` in `TestCase.setup`.
- Passing `update_expectations_on_fail = True` in `TestCase.verify_test_result`
...
- APIs based on data formats other than JSON and plain text
  - It's possible other formats could be used, but this would surely require more effort
- File uploads
- SOAP
- Websockets
- RPC
