...
class TestUtils(unittest.TestCase):
    ...
    def test_json_update__3_keys(self) -> None:
        actual = utils.json_update(j={'a': {'b': {'c': 1}}}, path='a.b.c', value=2)
        self.assertDictEqual({'a': {'b': {'c': 2}}}, actual)

    def test_json_update__3_keys_1_index(self) -> None:
        actual = utils.json_update(j={'a': {'b': {'c': [1, 2]}}}, path='a.b.c.[1]', value=3)
        self.assertDictEqual({'a': {'b': {'c': [1, 3]}}}, actual)

    def test_json_update__3_keys_2_indices(self) -> None:
        actual = utils.json_update(j={'a': {'b': {'c': [1, [2, 3]]}}}, path='a.b.c.[1].[1]', value=4)
        self.assertDictEqual({'a': {'b': {'c': [1, [2, 4]]}}}, actual)
    ...
    def test_json_update__1_index_1_key(self) -> None:
        actual = utils.json_update(j=[{'a': 1, 'b': 2}, 2, 3], path='[0].a', value=4)
        self.assertListEqual([{'a': 4, 'b': 2}, 2, 3], actual)

    def test_json_update__star_key(self) -> None:
        actual = utils.json_update(j=[{'a': 1, 'b': 2}, {'a': 3}, 4, {'c': 5}], path='*a', value=123)
        self.assertListEqual([{'a': 123, 'b': 2}, {'a': 123}, 4, {'c': 5, 'a': 123}], actual)

    def test_json_update__star_key_2(self) -> None:
        actual = utils.json_update(j=[{'a': 1}, {'a': 2}], path='*b', value=123)
        self.assertListEqual([{'a': 1, 'b': 123}, {'a': 2, 'b': 123}], actual)

    def test_json_update__1_index_star_key(self) -> None:
        actual = utils.json_update(j=[100, [{'a': 1, 'b': 2}, {'a': 3}, 4, {'c': 5}]], path='[1].*a', value=123)
        self.assertListEqual([100, [{'a': 123, 'b': 2}, {'a': 123}, 4, {'c': 5, 'a': 123}]], actual)

    def test_json_update__1_key_star_key(self) -> None:
        actual = utils.json_update(j={'z': [{'a': 1, 'b': 2}, {'a': 3}, 4, {'c': 5}], 'y': 4}, path='z.*a', value=123)
        self.assertDictEqual({'z': [{'a': 123, 'b': 2}, {'a': 123}, 4, {'c': 5, 'a': 123}], 'y': 4}, actual)

    def test_json_update__1_missing_key_star_key(self) -> None:
        actual = utils.json_update(j={'a': [{'b': 1}]}, path='c.*a', value=123)
        self.assertDictEqual({'a': [{'b': 1}]}, actual)
